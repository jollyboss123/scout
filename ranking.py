...
def score_rows(
    cands_texts, rows, limit, weights, type_boost_map, bbox=None, proximity_km=25.0
):
    ...
    for r in rows:
        ...
        for qtext in cands_texts:
            ...
            sim = max(
                fuzz.WRatio(q, target_local) if target_local else 0,
                fuzz.WRatio(q, target_en) if target_en else 0,
            )
            best_sim = max(best_sim, sim / 100.0)

        s = (
            (weights["similarity"] * best_sim)
